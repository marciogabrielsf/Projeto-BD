...
import { createReservation } from "@/app/services/reservations.service";
...
export default function AddReserveModal({ onRequestClose }: Props) {
	...
	const addButton = async (e: React.MouseEvent) => {
		...
		if (date && selectedClient !== 0 && selectedTable !== 0) {
			await createReservation(selectedClient, selectedTable, date);
			refetch();
			onRequestClose();
		} else {
